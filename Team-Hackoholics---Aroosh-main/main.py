...
async def _extract_onboarding_info(team_id: int, text: str) -> str:
    """
    ...
    prompt = (
        "You are an AI assistant helping a hackathon team onboard. "
        ...
        "In 'reply_message', provide a conversational reply. If they just said hello, greet them and ask for their deadline, headcount, and skills. "
        "If they provided some info, acknowledge it and ask for the missing parts. "
        "If they seem to have provided everything or a reasonable amount, acknowledge it and tell them to send their project concept next so you can review the scope.\n\n"
        f"Message:\n{text}"
    )
    try:
    ...
    default_reply = "Got it — noted your details. Send your project concept next and I'll review the scope."
    return parsed.get("reply_message", default_reply)
...
async def telegram_webhook(request: Request):
    try:
        ...
        if not (is_command or is_tagged):
            return {"ok": True}
        ...
        if team.get("headcount") is None or not team.get("skills"):
            reply_msg = await _extract_onboarding_info(team_id, text)
            await tg.send_message(chat_id, reply_msg)
            return {"ok": True}
